...
library(ggsci)
#Load data frame
TCGA.ov <- read_tsv(file = "./Desktop/Turbo TCGA/Ovarian_normal_Tumor.tsv")
TCGA.ov.copynum <- read_tsv(file = "./Desktop/Turbo TCGA/Ovarian_copynumber.tsv")
TCGA.ov.clinical <- read_tsv(file = "./Desktop/Turbo TCGA/Ovarian_ptdata.tsv")

TCGA.ov.clinical <- TCGA.ov.clinical %>% filter(!is.na(clinical.stage)
                                                & !is.na(grade))

TCGA.ov.clinical <- TCGA.ov.clinical %>% filter(clinical.stage != "Stage IC")
TCGA.ov.clinical <- TCGA.ov.clinical %>% filter(grade == "GX" |grade == "G3" |grade == "G2")

#Annotate the comparisson groups
ov.comparissons <- list(c("Normal Tissue","Primary Tumor"))

#Rename column titles to remove the underscores
names(TCGA.ov)[names(TCGA.ov) == "_sample_type"] <- "sample.type"
names(TCGA.ov.copynum)[names(TCGA.ov.copynum) == "DFNA5 (average)"] <- "DFNA5"
names(TCGA.ov.clinical)[names(TCGA.ov.clinical) == "DFNA5 (average)"] <- "DFNA5"
names(TCGA.ov.clinical)[names(TCGA.ov.clinical) == "clinical_stage"] <- "clinical.stage"
names(TCGA.ov.clinical)[names(TCGA.ov.clinical) == "neoplasm_histologic_grade"] <- "grade"
names(TCGA.ov.clinical)[names(TCGA.ov.clinical) == "ENSG00000105928.12"] <- "GSDME"
...
#Run some stats 
stat.test.ov <- TCGA.ov %>% wilcox_test(DFNA5 ~ sample.type)

stats <- stat.test.ov %>% add_xy_position(fun = "mean_sd", x = "sample.type")
  
#define a color pallette
pal <- c("#A034F0", "#159090")

#Generate plots using ggplot2 for gene expression
ggplot(TCGA.ov, aes(x = sample.type, y = DFNA5))+
  ggdist::stat_halfeye(
  ...
  labs(
    ...
    title = "TCGA Ovarian Serous Adenocarcinoma: Gasdermin E Expression"
  )+
...
ggplot(TCGA.ov.clinical, aes(x = GSDME, y = clinical.stage, fill = clinical.stage))+
